...
## Happy-datasets  ###
EXPRSSIONS_TYPE_happy =  ['0=Per1', '1=Per2' ]
...
def main(args):
    ...
    ###########################   Hand    ##########################
    image_list, label_list, nrof_classes, image_list_test, label_list_test, nrof_classes_test \
        = facenet_ext.get_image_paths_and_labels_hand(args.data_dir, args.labels_hand, args.nfold, args.ifold)

    # ### test case ###
    # image_list = image_list+image_list_test
    ...
    for i, label in enumerate(label_list):
        if label == 0:
            image_list_pers0.append(image_list[i])
            label_list_pers0.append(label)
        else:
            image_list_pers1.append(image_list[i])
            label_list_pers1.append(label)

    ### balance the test samples ####
    image_list_test_pers0 = []
    image_list_test_pers1 = []
    label_list_test_pers0 = []
    label_list_test_pers1 = []
    for i, label in enumerate(label_list_test):
        if label == 0:
            image_list_test_pers0.append(image_list_test[i])
            label_list_test_pers0.append(label)
        else:
            image_list_test_pers1.append(image_list_test[i])
            label_list_test_pers1.append(label)
    range_list = range(np.shape(label_list_test_pers0)[0])
    idx_epoch = random.sample(range_list, np.shape(label_list_test_pers1)[0])
    image_list_pers0_b = np.array(image_list_test_pers0)[idx_epoch]
    label_list_test_pers0_b = np.array(label_list_test_pers0)[idx_epoch]
    image_list_test = image_list_pers0_b.tolist() + image_list_test_pers1
    label_list_test = label_list_test_pers0_b.tolist() + label_list_test_pers1
    ...
    if args.evaluate_express:
        ...
        # ###########################   Hand    ##########################
        # image_list_test, label_list_test, usage_list_test, nrof_classes_test = facenet_ext.get_image_paths_and_labels_hand(args.data_dir,
        #                                                                                                 args.labels_hand,
        #                                                                                                   'Test',
        #                                                                                                   args.nfold,
        #                                                                                                   args.ifold)
    ...
    #
        ...
        labels_pers0 = ops.convert_to_tensor(label_list_pers0, dtype=tf.int32)
        range_size_pers0 = array_ops.shape(labels_pers0)[0]
        index_queue_pers0 = tf.train.range_input_producer(range_size_pers0, num_epochs=None,
                             shuffle=True, seed=None, capacity=32)
        index_dequeue_op_pers0 = index_queue_pers0.dequeue_many(args.batch_size*args.epoch_size, 'index_dequeue_pers0')

        labels_pers1 = ops.convert_to_tensor(label_list_pers1, dtype=tf.int32)
        range_size_pers1 = array_ops.shape(labels_pers1)[0]
        index_queue_pers1 = tf.train.range_input_producer(range_size_pers1, num_epochs=None,
                             shuffle=True, seed=None, capacity=32)
        index_dequeue_op_pers1 = index_queue_pers1.dequeue_many(args.batch_size*args.epoch_size, 'index_dequeue_pers1')
            ...
            while epoch < args.max_nrof_epochs:
                ...
                center_loss_, verifacc, learning_rate_ \
                    = train(args, sess, epoch, image_list, label_list, index_dequeue_op,
                            ...
                            phase_train_placeholder_expression, best_acc, label_list_test, index_dequeue_op_pers0,
                            index_dequeue_op_pers1, image_list_pers0, image_list_pers1, label_list_pers0, label_list_pers1)
                    ...
                    if args.evaluate_express:
                        ...
                        express_recog_images \
                            = evaluate_expression(sess, enqueue_op, image_paths_placeholder, labels_placeholder,
                                                  batch_size_placeholder, logits, label_batch, image_list_test,
...
def train(args, sess, epoch, image_list, label_list, index_dequeue_op, enqueue_op, image_paths_placeholder, labels_placeholder,
      ...
      loss, train_op, summary_op, summary_writer, regularization_losses, learning_rate_schedule_file, prelogits_center_loss,
          ...
          phase_train_placeholder_expression, best_acc, label_list_test, index_dequeue_op_pers0, index_dequeue_op_pers1,
          image_list_pers0, image_list_pers1, label_list_pers0, label_list_pers1):
    batch_number = 0
    ...
    # ### no random sampling for training set ###
    # label_epoch = np.array(label_list)
    # label_epoch = np.tile(label_epoch, args.epoch_size)
    # image_epoch = np.array(image_list)
    # image_epoch = np.tile(image_epoch, args.epoch_size)
    ...
    ### balance the training samples in the batch
    index_idx_epoch_pers0 = sess.run(index_dequeue_op_pers0)
    image_epoch_pers0 = np.array(image_list_pers0)[index_idx_epoch_pers0]
    label_epoch_pers0 = np.array(label_list_pers0)[index_idx_epoch_pers0]

    index_idx_epoch_pers1 = sess.run(index_dequeue_op_pers1)
    image_epoch_pers1 = np.array(image_list_pers1)[index_idx_epoch_pers1]
    label_epoch_pers1 = np.array(label_list_pers1)[index_idx_epoch_pers1]

    label_epoch = np.concatenate((label_epoch_pers0, label_epoch_pers1), axis=None)
    image_epoch = np.concatenate((image_epoch_pers0, image_epoch_pers1), axis=None)

    range_list = range(np.shape(label_epoch)[0])
    idx_epoch = random.sample(range_list, np.shape(image_epoch_pers0)[0])
    label_epoch = label_epoch[idx_epoch]
    image_epoch = image_epoch[idx_epoch]

    for i in range(10):
        print('index: %d' %idx_epoch[i])
        print('label_epoch: %d image_epoch: %s' % (label_epoch[i], image_epoch[i]))
    print('In this epoch including Person 1 samples: %d Person2 samples: %d' % (np.shape(label_epoch)[0]-sum(label_epoch), sum(label_epoch)))
    ...
    while batch_number < args.epoch_size:
        ...
        print(label_batch_)
        print('Training: Person 0: %d; Person 1: %d; batch_size: %d' % (
        label_batch_.shape[0] - sum(label_batch_), sum(label_batch_), label_batch_.shape[0]))
        print('+++++ Test +++++: Person 0: %d; Person 1: %d; batch_size: %d' % (
        len(label_list_test)- sum(label_list_test), sum(label_list_test), len(label_list_test)))
...
def evaluate_expression(sess, enqueue_op, image_paths_placeholder, labels_placeholder,
    ...
    nrof_batches = int(math.ceil(float(nrof_images / batch_size))) ## The floor division to get the maximum number of the complete batch
    ...
    # ### test tf and python ####
    # img_file = ops.convert_to_tensor(image_paths[0], dtype=tf.string)
    # file_contents = tf.read_file(img_file)
    # image_tf0 = tf.image.decode_png(file_contents)
    # image_tf1 = tf.image.resize_images(image_tf0, [args.image_size, args.image_size])
    # image_tf1.set_shape((args.image_size, args.image_size, 3))
    # image_tf3 = tf.image.per_image_standardization(image_tf1)
    #
    # image_ = sess.run(image_tf3)
    # image_test = facenet_ext.load_data(image_paths[0:2], True, False, args.image_size)
...
def parse_arguments(argv):
    ...
    parser.add_argument('--labels_hand', type=str,
