### Facenent for recognition of the seven expressions : neutral expression + six expressions : #########
### mingzuheng, 25/12/2017 #########
##
##############################  FACIAL EXPRESSION DATASETS   ##############################
### FER2013(unconstrained/webimages): 0=Angry, 1=Disgust, 2=Fear, 3=Happy, 4=Sad, 5=Surprise, 6=Neutral
###                                   Training: 28709 frames
...
## OULU-CASIA  ###
#XPRSSIONS_TYPE =  ['0=Anger', '1=Disgust', '2=Fear', '3=Happiness', '4=Sadness', '5=Surprise' ]
...
### FER2013 ###
EXPRSSIONS_TYPE_fer2013 =  ['0=Anger', '1=Disgust', '2=Fear', '3=Happiness', '4=Sadness', '5=Surprise', '6=Neutral']

### CK+  ###
EXPRSSIONS_TYPE_ckplus =  ['0=Neutral', '1=Anger', '2=Contempt', '3=Disgust', '4=Fear', '5=Happiness', '6=Sadness', '7=Surprise']

## OULU-CASIA  ###
EXPRSSIONS_TYPE_oulucasia =  ['0=Neutral', '1=Anger', '2=Disgust', '3=Fear', '4=Happiness', '5=Sadness', '6=Surprise' ]
#EXPRSSIONS_TYPE_oulucasia =  ['0=Anger', '1=Disgust', '2=Fear', '3=Happiness', '4=Sadness', '5=Surprise' ]

# ### SFEW  ###
EXPRSSIONS_TYPE_sfew =  ['0=Anger', '1=Disgust', '2=Fear', '3=Happiness', '4=Neutral', '5=Sadness', '6=Surprise']

## Fusion-datasets  ###
EXPRSSIONS_TYPE_fusion =  ['0=Neutral', '1=Anger', '2=Disgust', '3=Fear', '4=Happiness', '5=Sadness', '6=Surprise' ]

## Happy-datasets  ###
EXPRSSIONS_TYPE_happy =  ['0=Non-Happiness', '1=Happiness' ]

######## Current Expression labels #############
# EXPRSSIONS_TYPE = EXPRSSIONS_TYPE_fer2013
# EXPRSSIONS_TYPE = EXPRSSIONS_TYPE_ckplus
# EXPRSSIONS_TYPE = EXPRSSIONS_TYPE_oulucasia
# EXPRSSIONS_TYPE = EXPRSSIONS_TYPE_sfew
EXPRSSIONS_TYPE = EXPRSSIONS_TYPE_fusion
######## Current Expression labels ##############
...
def main(args):
    ...
    ###########################   GENKI4K+    ##########################
    image_list, label_list, usage_list, nrof_classes = facenet_ext.get_image_paths_and_labels_genki4k(args.labels_expression,
                                                                                                  'Training')
    ...
    ###########################   CK+    ##########################
    # image_list, label_list, usage_list, nrof_classes = facenet.get_image_paths_and_labels_ckplus(args.data_dir,
    #                                                                                               args.labels_expression,
    #                                                                                               'Training', args.nfold, args.ifold)

    ##########################   OULU-CASIA    ##########################
    #mage_list, label_list, usage_list, nrof_classes = facenet.get_image_paths_and_labels_oulucasia(args.data_dir,
    #                                                                                           args.labels_expression,
    #                                                                                           'Training', args.nfold,
    #                                                                                           args.ifold)

    # ############################   SFEW    ##########################
    # image_list, label_list, nrof_classes = facenet.get_image_paths_and_labels_sfew(args.data_dir, args.labels_expression)
    ...
    # usage_list = []
    # nrof_classes = 0
    # datasets=str.split(args.data_dir,';')
    # labels = str.split(args.labels_expression,';')
    # for i, dataset in enumerate(datasets):
    #     dataset_name = str.split(dataset,'/')[4]
    #     label = labels[i]
    #     label_name = str.split(label, '/')[4]
    #
    #     ### FER2013 ####
    #     if(dataset_name == 'fer2013' and label_name == 'fer2013'):
    #         image_list_i, label_list_i, _, nrof_classes_i = facenet.get_image_paths_and_labels_fer2013(dataset,
    #                                                                                                       label,
    #                                                                                                       'Training')
    #         idx_label_notexist, label_list_i_mapping = label_mapping(label_list_i, EXPRSSIONS_TYPE_fer2013, EXPRSSIONS_TYPE)
    #         if idx_label_notexist != []:
    #             image_list_i = np.asarray(image_list_i)
    #             label_list_i_mapping = np.asarray(label_list_i_mapping)
    #             image_list_i = np.delete(image_list_i, idx_label_notexist, None)
    #             label_list_i_mapping = np.delete(label_list_i_mapping, idx_label_notexist, None)
    #             image_list_i = image_list_i.tolist()
    #             label_list_i_mapping = label_list_i_mapping.tolist()
    #
    #         image_list += image_list_i
    #         label_list += label_list_i_mapping
    #         nrof_classes += nrof_classes_i
    #
    #     ### CK+ ####
    #     if(dataset_name == 'CK+' and label_name == 'CK+'):
    #         image_list_i, label_list_i, _, nrof_classes_i = facenet.get_image_paths_and_labels_ckplus(dataset,
    #                                                                                                     label,
    #                                                                                                     'Training', args.nfold,
    #                                                                                                     args.ifold)
    #         idx_label_notexist, label_list_i_mapping = label_mapping(label_list_i, EXPRSSIONS_TYPE_ckplus, EXPRSSIONS_TYPE)
    #         if idx_label_notexist != []:
    #             image_list_i = np.asarray(image_list_i)
    #             label_list_i_mapping = np.asarray(label_list_i_mapping)
    #
    ...
    #
    #     ### OULU-CASIA ####
    #     if(dataset_name == 'Oulu-Casia' and label_name =='Oulu-Casia'):
    #         image_list_i, label_list_i, _, nrof_classes_i = facenet.get_image_paths_and_labels_oulucasia(dataset,
    #                                                                                                     label,
    #                                                                                                     'Training', args.nfold,
    #                                                                                                     args.ifold)
    #         idx_label_notexist, label_list_i_mapping = label_mapping(label_list_i, EXPRSSIONS_TYPE_oulucasia, EXPRSSIONS_TYPE)
    #         if idx_label_notexist != []:
    #             image_list_i = np.asarray(image_list_i)
    #             label_list_i_mapping = np.asarray(label_list_i_mapping)
    #             image_list_i = np.delete(image_list_i, idx_label_notexist, None)
    ...
    #         nrof_classes += nrof_classes_i
    #     ### SFEW ####
    #     if (dataset_name == 'SFEW' and label_name =='SFEW'):
    #         image_list_i, label_list_i, nrof_classes_i = facenet.get_image_paths_and_labels_sfew(dataset, label)
    #         idx_label_notexist, label_list_i_mapping = label_mapping(label_list_i, EXPRSSIONS_TYPE_sfew, EXPRSSIONS_TYPE)
    #         if idx_label_notexist != []:
    #             image_list_i = np.asarray(image_list_i)
    #             label_list_i_mapping = np.asarray(label_list_i_mapping)
    #             image_list_i = np.delete(image_list_i, idx_label_notexist, None)
    ...
    #         nrof_classes += nrof_classes_i
    #
    # ### Re-labelling the dataset as binary class dataset: 'Happy' and 'unhappy'
    # k=0
    # label_list_idx_unhappy = []
    # label_list_idx_happy = []
    # for label in label_list:
    #     if label != 4: ## Happy in EXPRSSIONS_TYPE
    #         label = 0
    #         label_list_idx_unhappy.append(k)
    #     else:
    #         label = 1
    #         label_list_idx_happy.append(k)
    #     label_list[k] = label
    #     k += 1
    ...
    if args.evaluate_express:
        ...
        ###########################   GENKI4K+    ##########################
        image_paths_test, label_list_test, usage_list_test, nrof_classes_test = facenet_ext.get_image_paths_and_labels_genki4k(args.labels_expression, 'Test')

        ###########################   FER2013    ##########################
        # image_paths_test, label_list_test, usage_list_test, nrof_classes_test = facenet.get_image_paths_and_labels_fer2013(args.data_dir,
        #                                                                                               args.labels_expression,
        #                                                                                               'PublicTest')

        ###########################   CK+    ##########################
        # image_paths_test, label_list_test, usage_list_test, nrof_classes_test = facenet.get_image_paths_and_labels_ckplus(
        ...
        # ###########################   SFEW    ##########################
        # image_paths_test, label_list_test, nrof_classes_test = facenet.get_image_paths_and_labels_sfew(args.data_dir_test, args.labels_expression_test)

     #    dataset_name = str.split(args.data_dir_test, '/')[4]
     #    label_name = str.split(args.labels_expression_test, '/')[4]
     #    ##########################   FER2013    ##########################
     #    if (dataset_name == 'fer2013' and label_name == 'fer2013'):
     #        image_paths_test, label_list_test, usage_list_test, nrof_classes_test = facenet.get_image_paths_and_labels_fer2013(args.data_dir_test,
     #                                                                                                      args.labels_expression_test,
     #                                                                                                      'PublicTest')
	#     idx_label_notexist, label_list_i_mapping = label_mapping(label_list_test, EXPRSSIONS_TYPE_fer2013, EXPRSSIONS_TYPE)
	#     if idx_label_notexist != []:
     #            image_list_i = np.asarray(image_list_i)
     #            label_list_i_mapping = np.asarray(label_list_i_mapping)
     #            image_list_i = np.delete(image_list_i, idx_label_notexist, None)
     ...
     #            image_list_i = image_list_i.tolist()
     #            label_list_i_mapping = label_list_i_mapping.tolist()
     #        label_list_test = label_list_i_mapping
    #
     #    ##########################   CK+    ##########################
     #    if(dataset_name == 'CK+' and label_name == 'CK+'):
     #        image_paths_test, label_list_test, usage_list_test, nrof_classes_test = facenet.get_image_paths_and_labels_ckplus(
     #            args.data_dir_test, args.labels_expression_test, 'Test', args.nfold, args.ifold)
     #        idx_label_notexist, label_list_i_mapping = label_mapping(label_list_test, EXPRSSIONS_TYPE_ckplus, EXPRSSIONS_TYPE)
     #        if idx_label_notexist != []:
     #            image_list_i = np.asarray(image_list_i)
     #            label_list_i_mapping = np.asarray(label_list_i_mapping)
     #            image_list_i = np.delete(image_list_i, idx_label_notexist, None)
	...
	# ##########################   OULU-CASIA    ##########################
     ...
     #        image_paths_test, label_list_test, usage_list_test, nrof_classes_test = facenet.get_image_paths_and_labels_oulucasia(
     #           args.data_dir_test, args.labels_expression_test, 'Test', args.nfold, args.ifold)
     #        idx_label_notexist, label_list_i_mapping = label_mapping(label_list_test, EXPRSSIONS_TYPE_oulucasia, EXPRSSIONS_TYPE)
     #        if idx_label_notexist != []:
     #            image_list_i = np.asarray(image_list_i)
     #            label_list_i_mapping = np.asarray(label_list_i_mapping)
     #            image_list_i = np.delete(image_list_i, idx_label_notexist, None)
    ...
    #
     ...
     #    if (dataset_name == 'SFEW' and label_name =='SFEW'):
     #        image_paths_test, label_list_test, nrof_classes_test = facenet.get_image_paths_and_labels_sfew(args.data_dir_test, args.labels_expression_test)
     #        idx_label_notexist, label_list_i_mapping = label_mapping(label_list_test, EXPRSSIONS_TYPE_sfew, EXPRSSIONS_TYPE)
     #        if idx_label_notexist != []:
     #            image_list_i = np.asarray(image_list_i)
     #            label_list_i_mapping = np.asarray(label_list_i_mapping)
     #            image_list_i = np.delete(image_list_i, idx_label_notexist, None)
    ...
    #
        ...
        # labels_happy = ops.convert_to_tensor(label_list_idx_happy, dtype=tf.int32)
        # range_size_happy = array_ops.shape(labels_happy)[0]
        # index_queue_happy = tf.train.range_input_producer(range_size_happy, num_epochs=None,
        #                      shuffle=True, seed=None, capacity=32)
        # index_dequeue_op_happy = index_queue_happy.dequeue_many(args.batch_size*args.epoch_size, 'index_dequeue_happy')
        # labels_unhappy = ops.convert_to_tensor(label_list_idx_unhappy, dtype=tf.int32)
        # range_size_unhappy = array_ops.shape(labels_unhappy)[0]
        # index_queue_unhappy = tf.train.range_input_producer(range_size_unhappy, num_epochs=None,
        #                      shuffle=True, seed=None, capacity=32)
        # index_dequeue_op_unhappy = index_queue_unhappy.dequeue_many(args.batch_size*args.epoch_size, 'index_dequeue_unhappy')
        ...
        for var in restore_vars:
            ...
            paracnt += paranum
        print('The number of the loading parameters in the model(Inception Facenet) is %dM, ......the size is : %dM bytes'%(paracnt/1e6, parasize/1e6))
        ...
        for var in update_gradient_vars:
            ...
            parasize += paranum* sys.getsizeof(var.dtype)
            paracnt += paranum
        print('The number of the update parameters in the model is %dK, ......the size is : %dM bytes'%(paracnt/1e3, parasize/1e6))
        #update_gradient_vars0 = tf.global_variables()
        #update_gradient_vars = tf.trainable_variables()
        # Build the summary operation based on the TF collection of Summaries.
        summary_op = tf.summary.merge_all()

        train_op,grads, grads_clip = facenet.train(total_loss, global_step, args.optimizer,
        ...
        # Training and validation loop
            ...
            while epoch < args.max_nrof_epochs:
                ...
                # Train for one epoch
                step, train_each_expr_acc, softmax_acc_, grads_total_sum, loss, cross_entropy_mean_, Reg_loss,  \
                center_loss_, verifacc, learning_rate_ \
                    = train(args, sess, epoch, image_list, label_list, index_dequeue_op,
                            enqueue_op, image_paths_placeholder, labels_placeholder, learning_rate_placeholder,
                            ...
                            prelogits_center_loss, cross_entropy_mean, acc, val, far, centers_cts_batch_reshape,
                            softmax_acc, logits, keep_probability_placeholder, restore_vars, update_gradient_vars,
                            acc_expression, each_expr_acc, label_batch, express_probs_confus_matrix, log_dir, model_dir,
                            grads, grads_clip, cross_entropy, image_batch, learning_rate,
                            phase_train_placeholder_expression, best_acc)
                ...
                if not (epoch % 1 ):
                    if args.evaluate_express:
                        acc_expression, each_expr_acc, exp_cnt, expredict_cnt,  express_probs_confus_matrix, \
                        express_recog_images \
                            = evaluate_expression(sess, enqueue_op, image_paths_placeholder, labels_placeholder,
                                                  batch_size_placeholder, logits, label_batch, image_paths_test,
                                                  label_list_test, min(len(label_list_test), 100), log_dir, step,
                                                  summary_writer, keep_probability_placeholder,input_queue,
                                                  phase_train_placeholder_expression, phase_train_placeholder)

                        # acc_expression = evaluate_expression(sess, enqueue_op, image_paths_placeholder, labels_placeholder,
                        #                                      phase_train_placeholder, batch_size_placeholder, logits,
                        #                                      label_batch, image_list[0:2000], label_list[0:2000],
                        #                                      args.lfw_batch_size,
                        #                                      log_dir, step, summary_writer,
                        #                                      keep_probability_placeholder, input_queue)
                ...
                with open(os.path.join(log_dir, 'confusion_matrix.txt'), 'a') as f:
                    ...
                    f.write('loss: %2.3f  crossentropy: %2.3f  regloss: %2.3f  centerloss: %2.3f  verifAcc: %2.3f  lr: %1.1e \n' % (loss, cross_entropy_mean_, Reg_loss,  center_loss_, verifacc, learning_rate_))
                    line = ''
                    for idx, expr in enumerate(EXPRSSIONS_TYPE_happy):
                    ...
                    line = ''
                    for expr in EXPRSSIONS_TYPE_happy:
                        line += expr + ',  '
                    f.write(line + '\n')
...
def train(args, sess, epoch, image_list, label_list, index_dequeue_op, enqueue_op, image_paths_placeholder, labels_placeholder,
      ...
      loss, train_op, summary_op, summary_writer, regularization_losses, learning_rate_schedule_file, prelogits_center_loss,
          ...
          log_dir, model_dir, grads, grads_clip, cross_entropy, image_batch, learning_rate,
          phase_train_placeholder_expression, best_acc):
    batch_number = 0
    ...
    # index_idx_epoch_happy = sess.run(index_dequeue_op_happy)
    # idx_epoch_happy = np.array(label_list_idx_happy)[index_idx_epoch_happy]
    #
    # image_epoch_happy = np.array(image_list)[idx_epoch_happy]
    # label_epoch_happy = np.array(label_list)[idx_epoch_happy]
    #
    # index_idx_epoch_unhappy = sess.run(index_dequeue_op_unhappy)
    # idx_epoch_unhappy = np.array(label_list_idx_unhappy)[index_idx_epoch_unhappy]
    #
    # image_epoch_unhappy = np.array(image_list)[idx_epoch_unhappy]
    # label_epoch_unhappy = np.array(label_list)[idx_epoch_unhappy]
    #
    # label_epoch = np.concatenate((label_epoch_happy, label_epoch_unhappy), axis=None)
    # image_epoch = np.concatenate((image_epoch_happy, image_epoch_unhappy), axis=None)
    #
    # range_list = range(np.shape(label_epoch)[0])
    # idx_epoch = random.sample(range_list, np.shape(label_epoch_happy)[0])
    # label_epoch = label_epoch[idx_epoch]
    # image_epoch = image_epoch[idx_epoch]

    # for i in range(10):
    #     print('index: %d' %index_epoch[i])
    #     print('label_epoch: %d image_epoch: %s' % (label_epoch[i], image_epoch[i]))
    print('In this epoch including happy samples: %d unhappy samples: %d' % (sum(label_epoch), np.shape(label_epoch)[0]-sum(label_epoch)))
    ...
    while batch_number < args.epoch_size:
        ...
        if (batch_number % 100 == 0):
            #err, _, step, reg_loss, prelogits_center_loss_, cross_entropy_mean_, centers_cts_batch_reshape_, softmax_acc_, logits_, summary_str = sess.run([loss, train_op, global_step, regularization_losses, prelogits_center_loss, cross_entropy_mean, centers_cts_batch_reshape, softmax_acc, logits, summary_op], feed_dict=feed_dict)
            err, _, step, reg_loss, prelogits_center_loss_, cross_entropy_mean_, centers_cts_batch_reshape_, \
            softmax_acc_, logits_, label_batch_, grads_, grads_clip_, cross_entropy_, image_batch_, learning_rate_, \
            summary_str \
                = sess.run([loss, train_op, global_step, regularization_losses, prelogits_center_loss,
                            cross_entropy_mean, centers_cts_batch_reshape, softmax_acc, logits, label_batch, grads,
                            grads_clip, cross_entropy, image_batch, learning_rate, summary_op], feed_dict=feed_dict)
            summary_writer.add_summary(summary_str, global_step=step)
        else:
            #err, _, step, reg_loss, prelogits_center_loss_, cross_entropy_mean_, centers_cts_batch_reshape_, softmax_acc_, logits_ = sess.run([loss, train_op, global_step, regularization_losses,prelogits_center_loss, cross_entropy_mean, centers_cts_batch_reshape, softmax_acc, logits], feed_dict=feed_dict)
        ...
        print('The number of the update parameters in the model is %dK, ......the size is : %dM bytes' % (paracnt / 10e3, parasize / 10e6))
        ####################### check the state of the update weights/bias in the variables by summing up the values the dimensions of the variables ##########################
        ...
        # print('Epoch: [%d][%d/%d]\tTime %.3f\tLoss %2.3f\tCrossEntropy %2.3f\tRegLoss %2.3f\tCenterLoss_cosine %2.3f\tAcc %2.3f\tVal %2.3f\tFar %2.3f' %
        #       (epoch, batch_number+1, args.epoch_size, duration, err, cross_entropy_mean_, np.sum(reg_loss), prelogits_center_loss_, acc, val, far))
        print('Epoch: [%d][%d/%d]\tTime %.3f\tLoss %2.3f\tCrossEntropy %2.3f\tRegLoss %2.3f\tCenterLoss_l2 %2.3f\tAcc %2.3f\tVal %2.3f\tFar %2.3f\tsoftmax_acc %2.3f\ttest_expr_acc %2.3f\t lr %1.1e\tbest_acc %2.3f' %
        ...
        #print('Training each_expression_acc: 0=Angry %2.3f, 1=Disgust %2.3f, 2=Fear %2.3f, 3=Happy %2.3f, 4=Sad %2.3f, 5=Surprise %2.3f, 6=Neutral %2.3f' % (train_each_expr_acc[0], train_each_expr_acc[1], train_each_expr_acc[2], train_each_expr_acc[3], train_each_expr_acc[4], train_each_expr_acc[5],train_each_expr_acc[6]))
        #print ('Test each_expression_acc: 0=Angry %2.3f, 1=Disgust %2.3f, 2=Fear %2.3f, 3=Happy %2.3f, 4=Sad %2.3f, 5=Surprise %2.3f, 6=Neutral %2.3f'%(each_expr_acc[0], each_expr_acc[1], each_expr_acc[2], each_expr_acc[3], each_expr_acc[4], each_expr_acc[5], each_expr_acc[6]))
        
        print('Training each_expression_acc: ',end = '')
        for i, expr in enumerate(EXPRSSIONS_TYPE_happy):
...
def parse_arguments(argv):
    ...
    parser.add_argument('--model_def', type=str,
        help='Model definition. Points to a module containing the definition of the inference graph.', default='inception_resnet_v1.py')
